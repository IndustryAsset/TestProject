...
<c:if test="${progress == 'yes'}">
						...
						<tbody>
							...
							<c:forEach items="${ideas}" var="ideas" varStatus="loop">
								...
								<tr>
									...
									<c:out value="${ideas.approverAndSuperVisors}" />
									</c:if>
									</td>
									...
									<c:if test="${page == 'myidea' and ideaAndStatus[ideas.ideaId] != 'Pending Approval' and ideaAndStatus[ideas.ideaId] != 'Rejected'}">
  ...
  <div id="popupprogress" class="dialog">
           ...
           <tr> <c:if test="${page == 'myidea' and progress == 'yes'}">
           	<td><a href="markCompleted.htm?ideaId=${ideaId}" class="btn btn-info dropdown-toggle">Mark As Complete</a></td>
            </c:if></tr>
            </table>     
        </div>
